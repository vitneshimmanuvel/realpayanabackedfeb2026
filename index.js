...
// Pool error handling to prevent app crash on idle client errors
  ...
  return `
    <table border="1" cellpadding="6" cellspacing="0" style="border-collapse: collapse;">
      ${Object.entries(dataObj)
        .map(
          ([key, value]) =>
            `<tr><th align="left">${key}</th><td>${value}</td></tr>`,
        )
        .join("")}
    </table>
  `;
    ...
    const uploadStream = cloudinary.uploader.upload_stream(
      {
        ...
        eager_async: true,
        // Removed synchronous transformation to prevent timeouts on large files
...
app.post("/testimonials", videoUpload.single("video"), async (req, res) => {
  try {
    const { name, prefix, video_url } = req.body;

    // Check if either file or video_url is provided
    if (!req.file && !video_url) {
      return res
        .status(400)
        .json({ success: false, message: "Video file or URL is required" });
    }
    ...
    if (req.file) {
      ...
      finalCloudinaryId = cloudinaryResult.public_id;
    }
...
app.put("/testimonials/:id", videoUpload.single("video"), async (req, res) => {
  try {
    ...
    } else if (video_url && video_url !== videoUrl) {
       // If a new URL is provided (and it's different), update it
       // We might want to remove the old Cloudinary video if the new one is an external URL
       if (cloudinaryId) {
          try {
            await cloudinary.uploader.destroy(cloudinaryId, { resource_type: 'video' });
            console.log('🗑️ Old video deleted from Cloudinary (switched to URL)');
          } catch (err) {
             console.log('⚠️ Could not delete old video:', err.message);
          }
       }
       videoUrl = video_url;
       cloudinaryId = null; // No cloudinary ID for external URLs
    }
